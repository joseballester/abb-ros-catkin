...
{
  return RUN_AND_RETURN_RESULT(addBuffer(req.x, req.y, req.z, req.q0, req.qx, req.qy, req.qz, req.handpose), res.ret, res.msg, "ROBOT_CONTROLLER: Not able to add TCP pose to buffer the robot.");
}

SERVICE_CALLBACK_DEF(ExecuteBuffer)
{
  return RUN_AND_RETURN_RESULT(executeBuffer(req.simultaneous, req.useHandPose), res.ret, res.msg, "Not able to execute buffered TCP pose trajectory");
}
...
{
  return RUN_AND_RETURN_RESULT(executeJointPosBuffer(req.simultaneous), res.ret, res.msg, "Not able to execute buffered joint trajectories ");
...
bool RobotController::addBuffer(double x, double y, double z, double q0, double qx, double qy, double qz, double handpose)
{
  PREPARE_TO_TALK_TO_ROBOT
  strcpy(message, ABBInterpreter::addBuffer(x, y, z, q0, qx, qy, qz, handpose, randNumber).c_str());
  SEND_MSG_TO_ROBOT_AND_END
}

// Execute the TCP poses added to the buffer previously
bool RobotController::executeBuffer(bool simultaneous, bool useHandPose)
{
  PREPARE_TO_TALK_TO_ROBOT
  strcpy(message, ABBInterpreter::executeBuffer(simultaneous, useHandPose, randNumber).c_str());
  SEND_MSG_TO_ROBOT_AND_END
}
...
bool RobotController::executeJointPosBuffer(bool simultaneous)
{
  PREPARE_TO_TALK_TO_ROBOT
  strcpy(message, ABBInterpreter::executeJointPosBuffer(simultaneous, randNumber).c_str());
  SEND_MSG_TO_ROBOT_AND_END
}
